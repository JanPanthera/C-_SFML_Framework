...
namespace jp
{
	PauseScreenUi::PauseScreenUi(Game &p_GameRef) :
		m_PauseText(p_GameRef.getWindowRef(), DEFAULT_FONT),
		m_ResumeBtn(p_GameRef.getWindowRef(), DEFAULT_FONT, "Resume"),
	...
	void PauseScreenUi::onInit() {
		this->initializeButtons();
	}

	void PauseScreenUi::onInput(sf::Event &p_EventRef) {
		if (m_ResumeBtn.isMouseHovering() and Mouse::KeyPressed(KEY_LEFT_MOUSE_BTN))
		{
	...
	void PauseScreenUi::onUpdate(double p_ElapsedTime) {
		m_ResumeBtn.onUpdate(p_ElapsedTime);
		m_RestartBtn.onUpdate(p_ElapsedTime);
		m_QuitBtn.onUpdate(p_ElapsedTime);
	}

	void PauseScreenUi::onDraw(sf::RenderWindow &p_RenderWindowRef) {
		m_ResumeBtn.onDraw(p_RenderWindowRef);
		m_RestartBtn.onDraw(p_RenderWindowRef);
		m_QuitBtn.onDraw(p_RenderWindowRef);
	}

	void PauseScreenUi::onWindowResize() {
		...
		m_ResumeBtn.setScale(sf::Vector2f(_scaleX, _scaleY));
		m_ResumeBtn.setOrigin(sf::Vector2f(m_ResumeBtn.getTextButtonSize().x / 2, m_ResumeBtn.getTextButtonSize().y));
		m_ResumeBtn.setPosition(sf::Vector2f(m_WindowPtr->getWindowResolutionAsFloat().x / 2, m_RestartBtn.getTextButtonPosPoints().top - (m_PaddingBetweenButtons * _scaleY)));
	}

	void PauseScreenUi::initializeButtons() {
		...
		m_ResumeBtn.setTextStyle(sf::Text::Bold);
		m_ResumeBtn.setFontSize(m_ButtonFontSize);
		m_ResumeBtn.setOrigin(sf::Vector2f(m_ResumeBtn.getTextButtonSize().x / 2, m_ResumeBtn.getTextButtonSize().y));
		m_ResumeBtn.setPosition(sf::Vector2f(m_WindowPtr->getWindowResolutionAsFloat().x / 2, m_RestartBtn.getTextButtonPosPoints().top - (m_PaddingBetweenButtons * m_WindowPtr->getWindowInformations().s_WindowScaleFactor.y)));
		m_ResumeBtn.setOutlineColor(sf::Color(0, 0, 160, 255));
		m_ResumeBtn.setTextColor(sf::Color(0, 0, 180, 255));
		m_ResumeBtn.setHoverEffectSizeOffset(5);
		m_ResumeBtn.setOutlineThickness(1.f);
	}
